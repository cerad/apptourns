...
/// Cerad\Bundle\TournsBundle\Controller\Register\RegisterBaseController;


use Cerad\Bundle\PersonBundle\Validator\Constraints\USSF\ContractorId as FedIdConstraint;

/* ========================================================
 * This is intended to be the main traffic director
 */
class RegisterMainController extends RegisterBaseController
{   
    public function registerAction(Request $request, $slug, $op = null)
    {
        ...
        if (!$project) 
        {
            return $this->punt($request,'RegisterMain: No project found for: ' . $slug);
        }
               
        // Reroute based on op
        // Might want to check session for plan id as well?
        if (!$op || $op == 'step1')
        {
            $response = $this->forward('CeradTournsBundle:Register/RegisterStep1:register', array(
                'slug'    => $slug,
                'op'      => 'step1',
                'project' => $project,
            ));
